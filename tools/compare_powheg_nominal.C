void compare_powheg_nominal() {

  TString base = "selectionRoot/";

  TString filenominal = "POWHEG/ee/ee_ttbarsignalplustau_powheg_pu.root";
  TString filenominal2 = "POWHEG/ee/ee_ttbarsignalplustau_powheg.root";
  TString filepowhegbox = "POWHEG/ee/ee_ttbarsignalplustau_pwhgbox_nominal_gen_pu.root";
  TString filepowhegbox2 = "POWHEG/ee/ee_ttbarsignalplustau_pwhgbox_nominal_gen.root";
  ...
  TFile output("comparison_pwhg_nominal.root","recreate");
  ...
  for(std::vector<TString>::iterator hist = histograms.begin(); hist != histograms.end(); hist++) {
    ...
    leg->AddEntry(sample_powhegbox,"Nominal: POWHEG (GEN only)","l");
    leg->AddEntry(sample_powhegbox_pos,"Nominal: POWHEG (GEN only, noPU)","l");

    leg->AddEntry(sample_nominal,"Nominal: POWHEG (weighted)","l");
    leg->AddEntry(sample_nominal_pwhg,"Nominal: POWHEG (no weights)","l");
